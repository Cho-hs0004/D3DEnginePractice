...
struct App
{
    ...
    // Texture (Box 전용)
    UINT m_TexID = 0;
    std::unordered_map<UINT, ID3D11ShaderResourceView*> m_TexSRVs;
    std::unordered_map<ID3D11ShaderResourceView*, UINT> m_TexIDMaps;
    ComPtr<ID3D11ShaderResourceView> m_BoxTexSRV;
    ComPtr<ID3D11ShaderResourceView> m_GreenBoxTexSRV;
    ...
    Matrix                           m_GreenBoxWorld = Matrix::CreateTranslation(0, -1000, 0);
    ...
    void LoadBoxTexture()
    {
        ...
        CreateWICTextureFromFile(
            ...
            L"BoxTexture.png", nullptr, m_BoxTexSRV.GetAddressOf());

        m_TexIDMaps.insert(std::make_pair(m_BoxTexSRV.Get(), m_TexID));
        m_TexSRVs.insert(std::make_pair(m_TexID++, m_BoxTexSRV.Get()));

		CreateWICTextureFromFile(
			m_Device.Get(), m_Context.Get(),
			L"GreenBox.png", nullptr, m_GreenBoxTexSRV.GetAddressOf());

        m_TexIDMaps.insert(std::make_pair(m_GreenBoxTexSRV.Get(), m_TexID));
        m_TexSRVs.insert(std::make_pair(m_TexID++, m_GreenBoxTexSRV.Get()));
    ...
    void UpdateAndDraw()
    {
        ...
        for (auto& pObj : m_pObjs)
        {
			...
			m_Context->IASetVertexBuffers(0, 1, rf.m_VB.GetAddressOf(), &stride, &offset);
			m_Context->IASetIndexBuffer(rf.m_IB.Get(), DXGI_FORMAT_R16_UINT, 0);

			m_Context->VSSetShader(rf.m_VS.Get(), nullptr, 0);
			m_Context->PSSetShader(rf.m_PS.Get(), nullptr, 0);

			m_Context->PSSetShaderResources(0, 1, &m_TexSRVs[rf.TexID]);
    ...
    void CreateBox(Matrix mx, bool isGreenBox)
    {
        ...
        rf.m_VB = m_BoxVB;
        rf.m_IB = m_BoxIB;
        rf.m_IndexCnt = m_BoxIndexCount;
        rf.m_InputLayout = m_InputLayoutTex;

        rf.TexID = -1;

        if (isGreenBox)
        {
            if (m_TexIDMaps.find(m_GreenBoxTexSRV.Get()) != m_TexIDMaps.end())
            {
                rf.TexID = m_TexIDMaps[m_GreenBoxTexSRV.Get()];
            }
        }
        else
        {
			if (m_TexIDMaps.find(m_BoxTexSRV.Get()) != m_TexIDMaps.end())
			{
				rf.TexID = m_TexIDMaps[m_BoxTexSRV.Get()];
			}
        }
